import { AudioPlayerStatus, EndBehaviorType, NoSubscriberBehavior, NetworkingStatusCode, StreamType, VoiceConnectionDisconnectReason, VoiceConnectionStatus, createAudioPlayer, createAudioResource, entersState, joinVoiceChannel, } from "@discordjs/voice";
...
export class VoiceManager {
    ...
    onUtterance;
    sessions = new Map();
    constructor(client, config, logger, onUtterance) {
        this.client = client;
        this.config = config;
        this.logger = logger;
    ...
    async join(channel) {
        ...
        existing?.destroy();
        if (!channel.joinable) {
            throw new Error(`Discord reports voice channel "${channel.name}" is not joinable. ` +
        ...
        try {
            await entersState(connection, VoiceConnectionStatus.Ready, this.config.voiceJoinTimeoutMs);
        }
        catch (error) {
            ...
            throw new Error(`Voice connection did not become ready within ${this.config.voiceJoinTimeoutMs}ms. ` +
                `Last state: ${formatVoiceConnectionState(finalState)}.`, { cause: error });
        }
        let session;
        session = new VoiceSession(this.client, channel.guild.id, channel.id, connection, this.config, this.logger, this.onUtterance, () => {
...
function describeVoiceConnectionState(state) {
    ...
    if ("closeCode" in state)
        details.closeCode = state.closeCode;
    return details;
}
function formatVoiceConnectionState(state) {
    const parts = [state.status];
    if (state.networking !== undefined)
        parts.push(`networking=${state.networking}`);
    if (state.disconnectReason !== undefined)
        parts.push(`reason=${state.disconnectReason}`);
...
class VoiceSession {
    ...
    destroyed = false;
    constructor(client, guildId, channelId, connection, config, logger, onUtterance, onDestroyed) {
        this.client = client;
        ...
        this.player.on(AudioPlayerStatus.Idle, () => {
            if (this.destroyed)
                return;
            this.suppressCaptureUntil = Math.max(this.suppressCaptureUntil, Date.now() + this.config.voiceFeedbackCooldownMs);
    ...
    playNext() {
        ...
        this.suppressCaptureUntil = Math.max(this.suppressCaptureUntil, Date.now() + estimatedDurationMs + this.config.voiceFeedbackCooldownMs);
    ...
    async capture(userId) {
        ...
        decoder.on("data", (chunk) => {
            ...
            if (size > maximumBytes) {
                discarded = true;
                this.logger.warn(`Discarded an utterance over ${this.config.maxUtteranceSeconds}s in guild ${this.guildId}`);
                opusStream.destroy();
                decoder.destroy();
                finish();
                return;
