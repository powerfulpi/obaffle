...
export function analyzePcm16(pcm: Buffer): PcmActivity {
  ...
  return {
    durationMs:
      (pcm.length / (PCM_SAMPLE_RATE * PCM_CHANNELS * PCM_BYTES_PER_SAMPLE)) * 1_000,
...
export function pcmStereoToWav(pcm: Buffer): Buffer {
...
export function pcmMonoToStereo(pcm: Buffer): Buffer {
...
export function truncateUtf16(text: string, maximum: number): string {
